...
router.get('/bestsellers', async (req, res, next) => {
  try {
    const books = await getRepository(Book).findByIds([11, 12, 21, 13, 14, 10, 9, 8, 7, 6]);
...
router.get('/latests', async (req, res, next) => {
  try {
    const books = await getRepository(Book).findByIds([9, 3, 7, 17, 24, 1, 2, 4, 5, 22]);
...
router.get('/:id', async (req, res, next) => {
  try {
    ...
    const book = await getRepository(Book).createQueryBuilder('b')
      .select([
